...
DM the **your Kiro bot** bot. The model is simple:
...
| In a thread | `!verbose` | Toggle full tool trace on/off for this session |
...
- **Quiet by default.** Replies show just Kiro's answer — the tool trace (commands run, files read, raw command output) is hidden so threads stay readable. Add `-v` to `!new` (e.g. `!new -v api <task>`), set `verbose=true`, or send `!verbose` in a thread to see the full activity.
- **Formatted for Slack.** Output is rendered with the `slackify-markdown` library (Markdown → Slack mrkdwn: `**bold**` → *bold*, headings → bold, `-` → •, `[text](url)` → clickable links). Code blocks are preserved verbatim.
