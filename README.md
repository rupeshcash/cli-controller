...
Drive AI coding agents from **Slack** (or a local **web cockpit**) — from your phone, without SSH. Pick a **brain** (Kiro or Cline today; Claude Code / Codex later), talk to it in a thread, and let a **controller** handle sessions, routing, and memory on your behalf. Local-first, single-user, no server.

```
you (phone) ──▶ Slack thread ──▶ controller ──▶ Kiro / Cline on your machine ──▶ reply in the thread
```
...
- **Thread = session.** Send a message → the *controller* routes it (which repo, brain, agent, model) and opens a session in a Slack thread; reply in the thread to continue. Each new top-level message is a separate, parallel session.
- **`!` talks to the controller, bare text talks to the agent.** Inside a live thread, a plain reply goes to the coding agent; `!<anything>` summons the controller in natural language — `!use cline`, `!switch to opus`, `!abort`, `!recall the auth bug session`. Fast commands (`!abort`, `!status`, `!end`) stay instant and LLM-free.
- **Multi-brain.** Kiro and Cline today, behind one adapter contract — adding a brain is one file. Choose per session.
- **Persistent controller memory.** Every turn is captured to an ever-persistent local store; the controller can search and cross-reference past sessions (`!recall`, or the cockpit's `/api/memory`).
...
Single source of truth: [`plans/architecture/architecture.md`](plans/architecture/architecture.md) (compact, current). Two sides, one boundary: the **Controller** (the interface/management agent — routing, sessions, memory, `!` admin) drives the **CLIs / Brains** (Kiro, Cline) across a single **brain-adapter** integration contract (`core/brain`). Layered: **Interfaces** (Slack, web) → **Controller** → **core** (runner, brain registry, memory) → **Brains/CLIs** (Kiro, Cline). Memory design + the controller model: [`plans/architecture/arch_2_sequence.md`](plans/architecture/arch_2_sequence.md).
