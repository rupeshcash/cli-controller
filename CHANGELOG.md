...
### Fixed
- **Cross-session contamination.** A fresh Kiro turn discovers its own session id by diffing `kiro-cli --list-sessions` before/after the spawn (headless Kiro reports no id, and `kiro-cli` has no create-with-id flag). The old code took the first new session in the cwd and assumed it was ours, so when another session was created in the same cwd during the turn window (a second Slack thread, or a terminal `kiro-cli` in that dir) it could latch two threads onto one session and answer from the wrong context. Capture is now strict: attribute a session only when exactly one new one appeared, validated as a UUID (this also fixes a stray `"the"` sessionId). Fresh-session creation is serialized per cwd for the brief creation window (not the whole turn), and the id is captured as soon as it appears. Contamination is now impossible; worst case under concurrency is a missed capture (non-corrupting). Covered by `slack-bridge/test/capture.test.js`.
